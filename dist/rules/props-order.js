...
Object.defineProperty(exports, "propsOrderRule", {
    enumerable: true,
    get: function() {
        return propsOrderRule;
    }
});
const _utils = require("@typescript-eslint/utils");
const _isChakraElement = require("../lib/isChakraElement");
const _getPriorityIndex = require("../lib/getPriorityIndex");
...
const propsOrderRule = {
    ...
    create: (ctx)=>{
        ...
        const parserServices = !(option === null || option === void 0 ? void 0 : option.applyToAllComponents) ? (0, _eslintutils.getParserServices)(ctx, false) : undefined;
        return {
            JSXOpeningElement (node) {
                if (parserServices !== undefined && !(0, _isChakraElement.isChakraElement)(node, parserServices)) {
                    return;
                }
                const config = {
                ...
                for (const [index, attribute] of node.attributes.entries()){
                    ...
                    if (sortedAttribute.type !== _utils.AST_NODE_TYPES.JSXAttribute || sortedAttribute.name.name !== attribute.name.name) {
                        report({
                            node: node,
